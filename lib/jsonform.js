...
var getSchemaKey = function(schema,key) {
  ...
  var schemaKey = key
    ...
    .replace(/\[[0-9]*\]/g, '.items');
  var schemaDef = jsonform.util.getObjKey(properties, schemaKey, true);
  if (schemaDef && schemaDef.$ref) {
    throw new Error('JSONForm does not yet support schemas that use the ' +
      '$ref keyword. See: https://github.com/joshfire/jsonform/issues/54');
  }
  if(! schemaDef.required && typeof properties.required ===  "object" && Array.isArray(properties.required)) {
    var propertyName = schemaKey.split('.').pop();
    if(properties.required.indexOf(propertyName) !== -1) {
