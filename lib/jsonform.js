 ...
 *  - the AJV library for validation purpose
 *
...
jsonform.fieldTemplate = function(inner) {
  return '<div ' +
    ...
    '<%= (node.schemaElement && node.schemaElement._required && (node.schemaElement.type !== "boolean") ? " jsonform-required" : "") %>' +
    '<%= (node.readOnly ? " jsonform-readonly" : "") %>' +
    '<%= (node.disabled ? " jsonform-disabled" : "") %>' +
    '">' +
    '<% if (!elt.notitle) { %>' +
      '<label class="font-weight-bold" for="<%= node.id %>"><%= node.title ? node.title : node.name %></label>' +
    '<% } %>' +
    '<div class="controls">' +
      ...
      '<% } %>' +
      '<% if (node.description) { %>' +
        '<span class="help-block text-secondary"><%= node.description %></span>' +
      '<% } %>' +
...
var inputFieldTemplate = function (type) {
  return {
    'template': '<input type="' + type + '" ' +
      ...
      '<%= (node.schemaElement && node.schemaElement._required && (node.schemaElement.type !== "boolean") ? " required=\'required\'" : "") %>' +
...
jsonform.elementTypes = {
  ...
  'range': {
    'template': '<input type="range" ' +
      ...
      ' max=<%= range.max %>' +
      ' step=<%= range.step %>' +
      '<%= (node.schemaElement && node.schemaElement._required ? " required=\'required\'" : "") %>' +
      ' />',
    'fieldtemplate': true,
    'inputfield': true,
...
var getSchemaKey = function(schema,key) {
  ...
  // Use required behaviour from draft-07
  // Use required behaviour from draft-07
  if(schemaDef && schemaDef.type && schemaDef.type !== "object") {
    var parentSchemaKeyArray = schemaKey.split('.');
    if(parentSchemaKeyArray.length > 0) {
      var propertyName = parentSchemaKeyArray.pop();
      parentSchemaKeyArray.pop();
      var parentSchemaDef = null;
      if(!parentSchemaKeyArray.length) {
        parentSchemaDef = schema;
      } else {
        parentSchemaDef = jsonform.util.getObjKey(schema, parentSchemaKeyArray.join('.'), true);
      }
      if (parentSchemaDef && typeof parentSchemaDef.required === "object" &&
          Array.isArray(parentSchemaDef.required) && parentSchemaDef.required.indexOf(propertyName) !== -1) {
        schemaDef._required = true;
      }
    }
  }
  return schemaDef;
...
formTree.prototype.validate = function(noErrorDisplay) {
  ...
  if (typeof options.validate!="object") {
      // Use https://github.com/ajv-validator/ajv in validate callback option:
      // import Ajv from 'ajv';
      ...
      // }
      var v = options.validate(values, this.formDesc.schema);
...
formTree.prototype.hasRequiredField = function () {
  var parseElement = function (element) {
    ...
    if (element._required && (element.type !== 'boolean')) {
      return element;
    }
...
$.fn.jsonFormErrors = function(errors, options) {
  ...
  for (var i = 0; i < errors.length; i++) {
    // Use https://github.com/ajv-validator/ajv
    // Remove leading point from dataPath error object attribute
    var key = errors[i].dataPath.substring(1);
